        ...
        if(packet.stream_index == stream_in->index) {
            ...
            if(frameInterval > 0) {
                if(encode_count == 0 || decode_count-last_encode_index >= frameInterval) {
                    shouldEncode = true;
                    FUNLOG(Info, "%s should cut one frame now! frameInterval=%d, decode_count=%ld, last_encode_index=%ld", 
                ...
                if(encode_count == 0 || compare_timestamp(diff_pts, stream_in->time_base, timeInterval) >= 0) {
                    shouldEncode = true;
                    FUNLOG(Info, "%s should cut one frame now! diff_pts*(timebase.num/timebase.den)=%ld*(%d/%d)  >= timeInterval=%d", 
