...
bool AudioProcessor::decodeAudio(int sampleRate, int channels, std::string &outbuf)
{
    ...
    while(result) {
        ...
        auto _defer_codec = utility::defer([&frame_list, &data_present]() { free_frame_list(frame_list, data_present); });
        ...
        for(int i = 0; i < data_present; i++) {
            ...
            //转码
            int ret_sample = swr_convert(convert_ctx, out_buffer, out_nb_samples, (const uint8_t **)frame->data, frame->nb_samples);
            if(ret_sample < 0) {
