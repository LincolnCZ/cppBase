...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (pkt.pts == AV_NOPTS_VALUE) {
            //Write PTS
            AVRational time_base1 = ifmt_ctx->streams[videoStreamIndex]->time_base;
            //Duration between 2 frames (us)
            int64_t calc_duration = (double) AV_TIME_BASE / av_q2d(ifmt_ctx->streams[videoStreamIndex]->r_frame_rate);
            //Parameters
        ...
        if (pkt.stream_index == videoStreamIndex) {
            AVRational time_base = ifmt_ctx->streams[videoStreamIndex]->time_base;
