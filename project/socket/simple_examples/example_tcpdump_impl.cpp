...
int main(int argc, char *argv[]) {
    ...
    if ((sock = socket(PF_PACKET, SOCK_RAW, 768/*htons(ETH_P_ALL)*/)) < 0) {
    ...
    while (1) {
        ...
        printf("acquire content len: %d\n", len);

        mac_hdr = reinterpret_cast<eth_hdr *>(buffer);
        ip_hdr = reinterpret_cast<iphdr *>(buffer + sizeof(struct ethhdr));
